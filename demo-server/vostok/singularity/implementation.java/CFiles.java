...
private static java.lang.String Name(byte[] name, int ofs) {
    java.lang.String n;
    n = O7.string(name, ofs);
    if (!"/dev/urandom".equals(n) && !"/dev/random".equals(n)) {
        n = "";
    }
    return n;
}
...
public static final File Open(byte[] name, int ofs, byte[] mode) {
    ...
    try {
        ...
        fc = new java.io.RandomAccessFile(Name(name, ofs), smode);
        f = new File();
        f.fc = fc;
...
public static final boolean Remove(byte[] name, int ofs) {
    boolean ok;
    try {
        ok = new java.io.File(Name(name, ofs)).delete();
...
public static final boolean Exist(byte[] name, int ofs) {
    ...
    exist = true;
    try {
        new java.io.RandomAccessFile(Name(name, ofs), "r").close();
