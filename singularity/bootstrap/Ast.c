...
extern void Ast_FormalParam_s_undef(struct Ast_FormalParam_s *r) {
	Ast_RVar_undef(&r->_);
	r->isVar = O7C_BOOL_UNDEF;
}
o7c_tag_t Ast_RProcType_tag;
...
static o7c_tag_t FormalProcType_s_tag;
...
extern void Ast_RDeclarations_undef(struct Ast_RDeclarations *r) {
	...
	r->vars = NULL;
	r->procedures = NULL;
	r->recordForwardCount = O7C_INT_UNDEF;
	r->stats = NULL;
}
...
extern void Ast_RModule_undef(struct Ast_RModule *r) {
	...
	r->provider = NULL;
	r->script = O7C_BOOL_UNDEF;
	r->errorHide = O7C_BOOL_UNDEF;
	r->handleImport = O7C_BOOL_UNDEF;
	r->import_ = NULL;
	r->fixed = O7C_BOOL_UNDEF;
	r->errors = NULL;
	r->errLast = NULL;
}
...
extern void Ast_PutChars(struct Ast_RModule *m, struct StringStore_String *w, o7c_tag_t w_tag, int s_len0, o7c_char s[/*len0*/], int begin, int end) {
	if (o7c_cmp(begin, 0) >=  0) {
		StringStore_Put(&m->store, StringStore_Store_tag, &(*w), w_tag, s_len0, s, begin, end);
	} else {
		StringStore_Put(&m->store, StringStore_Store_tag, &(*w), w_tag, 7, (o7c_char *)"#error", 0, 5);
	}
}

static void NodeInit(struct Ast_Node *n, o7c_tag_t n_tag, int id) {
	assert((o7c_cmp(Ast_NoId_cnst, id) <=  0) && (o7c_cmp(id, LastId_cnst) <=  0) || (o7c_cmp(Scanner_PredefinedFirst_cnst, id) <=  0) && (o7c_cmp(id, Scanner_PredefinedLast_cnst) <=  0));
	V_Init(&(*n)._, n_tag);
	(*n).id = id;
...
extern void Ast_NodeSetComment(struct Ast_Node *n, o7c_tag_t n_tag, struct Ast_RModule *m, int com_len0, o7c_char com[/*len0*/], int ofs, int end) {
	assert(!StringStore_IsDefined(&(*n).comment, StringStore_String_tag));
	Ast_PutChars(m, &(*n).comment, StringStore_String_tag, com_len0, com, ofs, end);
}

extern void Ast_DeclSetComment(struct Ast_RDeclaration *d, int com_len0, o7c_char com[/*len0*/], int ofs, int end) {
	Ast_NodeSetComment(&(*d)._, Ast_RDeclaration_tag, d->module, com_len0, com, ofs, end);
}

extern void Ast_ModuleSetComment(struct Ast_RModule *m, int com_len0, o7c_char com[/*len0*/], int ofs, int end) {
	Ast_NodeSetComment(&(*m)._._._, Ast_RModule_tag, m, com_len0, com, ofs, end);
}
...
static void DeclConnect(struct Ast_RDeclaration *d, struct Ast_RDeclarations *ds, int name_len0, o7c_char name[/*len0*/], int start, int end) {
	assert(d != NULL);
	assert(name[0] != 0x00u);
	...
	ds->end = d;
	Ast_PutChars(d->module, &d->name, StringStore_String_tag, name_len0, name, start, end);
}
...
static void DeclarationsConnect(struct Ast_RDeclarations *d, struct Ast_RDeclarations *up, int name_len0, o7c_char name[/*len0*/], int start, int end) {
...
extern struct Ast_RModule *Ast_ModuleNew(int name_len0, o7c_char name[/*len0*/], int begin, int end, struct Ast_RProvider *p) {
	struct Ast_RModule *m = NULL;

	O7C_NEW(&m, Ast_RModule);
	NodeInit(&(*m)._._._, Ast_RModule_tag, Ast_NoId_cnst);
	...
	Ast_PutChars(m, &m->_._.name, StringStore_String_tag, name_len0, name, begin, end);
	m->_._.module = m;
	m->provider = p;
	m->errorHide = true;
	m->handleImport = false;
	m->script = false;
	Log_Str(8, (o7c_char *)"Module ");
	Log_Str(StringStore_BlockSize_cnst + 1, m->_._.name.block->s);
	Log_StrLn(2, (o7c_char *)" ");
	return m;
}
...
extern struct Ast_RModule *Ast_ScriptNew(struct Ast_RProvider *p) {
	...
	m = Ast_ModuleNew(8, (o7c_char *)"script ", 0, 6, p);
	m->script = true;
	return m;
}

extern struct Ast_RModule *Ast_GetModuleByName(struct Ast_RModule *host, int name_len0, o7c_char name[/*len0*/], int ofs, int end) {
	return host->provider->get(host->provider, host, name_len0, name, ofs, end);
}
...
extern void Ast_ImportEnd(struct Ast_RModule *m) {
	assert(o7c_bl(m->handleImport));
	m->handleImport = false;
}

static int ImportAdd_Load(struct Ast_RModule **res, struct Ast_RModule *host, int buf_len0, o7c_char buf[/*len0*/], int realOfs, int realEnd) {
	...
	l = 0;
	ret = StringStore_CopyChars(TranslatorLimits_MaxLenName_cnst, n, &l, buf_len0, buf, realOfs, realEnd);
	assert(o7c_bl(ret));
	/* TODO сделать загрузку модуля из символьного файла */
	Log_Str(15, (o7c_char *)"Модуль '");
	Log_Str(TranslatorLimits_MaxLenName_cnst, n);
	Log_StrLn(25, (o7c_char *)"' загружается");
	(*res) = Ast_GetModuleByName(host, buf_len0, buf, realOfs, realEnd);
	if ((*res) == NULL) {
		(*res) = Ast_ModuleNew(buf_len0, buf, realOfs, realEnd, host->provider);
		err = Ast_ErrImportModuleNotFound_cnst;
	} else if ((*res)->errors != NULL) {
		err = Ast_ErrImportModuleWithError_cnst;
	} else if (o7c_bl((*res)->handleImport)) {
		err = Ast_ErrImportLoop_cnst;
	} else {
		err = Ast_ErrNo_cnst;
	}
	Log_Str(30, (o7c_char *)"Модуль получен: ");
	Log_Int((int)((*res) != NULL));
	Log_Ln();
	return err;
}

static o7c_bool ImportAdd_IsDup(struct Ast_Import_s *i, int buf_len0, o7c_char buf[/*len0*/], int nameOfs, int nameEnd, int realOfs, int realEnd) {
	return StringStore_IsEqualToChars(&i->_.name, StringStore_String_tag, buf_len0, buf, nameOfs, nameEnd) || (o7c_cmp(realOfs, nameOfs) !=  0) && ((o7c_cmp(i->_.name.ofs, i->_.module->_._.name.ofs) !=  0) || (i->_.name.block != i->_.module->_._.name.block)) && StringStore_IsEqualToChars(&i->_.module->_._.name, StringStore_String_tag, buf_len0, buf, realOfs, realEnd);
}

extern int Ast_ImportAdd(struct Ast_RModule *m, int buf_len0, o7c_char buf[/*len0*/], int nameOfs, int nameEnd, int realOfs, int realEnd) {
	...
	if (StringStore_IsEqualToChars(&m->_._.name, StringStore_String_tag, buf_len0, buf, realOfs, realEnd)) {
		err = Ast_ErrImportSelf_cnst;
	} else {
		while ((i != NULL) && !ImportAdd_IsDup(O7C_GUARD(Ast_Import_s, &i), buf_len0, buf, nameOfs, nameEnd, realOfs, realEnd)) {
			i = i->next;
		}
		if (i != NULL) {
			err = Ast_ErrImportNameDuplicate_cnst;
		} else {
			O7C_NEW(&imp, Ast_Import_s);
			NodeInit(&(*imp)._._, Ast_Import_s_tag, Ast_IdImport_cnst);
			DeclConnect(&imp->_, &m->_, buf_len0, buf, nameOfs, nameEnd);
			imp->_.mark = true;
			if (m->import_ == NULL) {
				m->import_ = imp;
			}
			err = ImportAdd_Load(&imp->_.module, m, buf_len0, buf, realOfs, realEnd);
		}
	}
	return err;
}

static struct Ast_RDeclaration *SearchName(struct Ast_RDeclaration *d, int buf_len0, o7c_char buf[/*len0*/], int begin, int end) {
	if (o7c_cmp(begin, 0) <  0) {
		d = NULL;
	} else {
		while ((d != NULL) && !StringStore_IsEqualToChars(&d->name, StringStore_String_tag, buf_len0, buf, begin, end)) {
			assert(!(o7c_is(d, Ast_RModule_tag)));
			d = d->next;
		}
	}
	if ((d != NULL) && false) {
		Log_Str(37, (o7c_char *)"Найдено объявление ");
		while (o7c_cmp(begin, end) !=  0) {
			Log_Char(buf[o7c_ind(buf_len0, begin)]);
			begin = o7c_mod((o7c_add(begin, 1)), (o7c_sub(buf_len0, 1)));
		}
		Log_Str(7, (o7c_char *)" id = ");
		Log_Int(d->_.id);
		Log_Ln();
	}
	return d;
...
static struct Ast_RDeclaration *DeclarationLineSearch(struct Ast_RDeclarations *ds, int buf_len0, o7c_char buf[/*len0*/], int begin, int end) {
	struct Ast_RDeclaration *d = NULL;

	d = SearchName(ds->start, buf_len0, buf, begin, end);
	if ((d == NULL) && (o7c_is(ds, Ast_RProcedure_tag))) {
		d = SearchName(&O7C_GUARD(Ast_RProcedure, &ds)->_.header->params->_._, buf_len0, buf, begin, end);
	}
	return d;
}

static int CheckNameDuplicate(struct Ast_RDeclarations *ds, int buf_len0, o7c_char buf[/*len0*/], int begin, int end) {
	int err = O7C_INT_UNDEF;

	if (DeclarationLineSearch(ds, buf_len0, buf, begin, end) != NULL) {
		err = Ast_ErrDeclarationNameDuplicate_cnst;
	} else if (o7c_bl(ds->_.module->errorHide) && (ds->_.up != NULL) && (DeclarationLineSearch(&ds->_.module->_, buf_len0, buf, begin, end) != NULL)) {
...
extern int Ast_ConstAdd(struct Ast_RDeclarations *ds, int buf_len0, o7c_char buf[/*len0*/], int begin, int end) {
	struct Ast_Const_s *c = NULL;
	int err = O7C_INT_UNDEF;

	assert(!ds->_.module->fixed);
	err = CheckNameDuplicate(ds, buf_len0, buf, begin, end);
	O7C_NEW(&c, Ast_Const_s);
	NodeInit(&(*c)._._, Ast_Const_s_tag, Ast_IdConst_cnst);
...
static void TypeAdd_MoveForwardDeclToLast(struct Ast_RDeclarations *ds, struct Ast_Record_s *rec) {
	...
	ds->recordForwardCount = o7c_sub(ds->recordForwardCount, 1);
	assert(o7c_cmp(ds->recordForwardCount, 0) >=  0);
}

extern int Ast_TypeAdd(struct Ast_RDeclarations *ds, int buf_len0, o7c_char buf[/*len0*/], int begin, int end, struct Ast_RType **td) {
	struct Ast_RDeclaration *d = NULL;
	int err = O7C_INT_UNDEF;

	assert(!ds->_.module->fixed);
	d = DeclarationLineSearch(ds, buf_len0, buf, begin, end);
	if (((d == NULL) || (o7c_cmp(d->_.id, Ast_IdRecordForward_cnst) ==  0)) || (d == NULL) && ((ds->_.up == NULL) || (DeclarationLineSearch(&ds->_.module->_, buf_len0, buf, begin, end) == NULL))) {
		err = Ast_ErrNo_cnst;
	} else if (d == NULL) {
		err = Ast_ErrDeclarationNameHide_cnst;
	} else {
...
extern int Ast_CheckUndefRecordForward(struct Ast_RDeclarations *ds) {
	...
	if (o7c_cmp(ds->recordForwardCount, 0) ==  0) {
		err = Ast_ErrNo_cnst;
	} else {
...
static void ChecklessVarAdd(struct Ast_RVar **v, struct Ast_RDeclarations *ds, int buf_len0, o7c_char buf[/*len0*/], int begin, int end) {
	O7C_NEW(&(*v), Ast_RVar);
	NodeInit(&(*(*v))._._, Ast_RVar_tag, Ast_IdVar_cnst);
...
extern int Ast_VarAdd(struct Ast_RDeclarations *ds, int buf_len0, o7c_char buf[/*len0*/], int begin, int end) {
...
static void TInit(struct Ast_RType *t, int id) {
	NodeInit(&(*t)._._, Ast_RType_tag, id);
	DeclInit(&t->_, NULL);
	t->array_ = NULL;
}

extern struct Ast_RProcType *Ast_ProcTypeNew(o7c_bool forType) {
	struct Ast_RProcType *p = NULL;
	struct FormalProcType_s *fp = NULL;

	if (o7c_bl(forType)) {
		O7C_NEW(&fp, FormalProcType_s);
		p = (&(fp)->_);
	} else {
		O7C_NEW(&p, Ast_RProcType);
	}
...
static void ParamAddPredefined(struct Ast_RProcType *proc, struct Ast_RType *type, o7c_bool isVar) {
	struct Ast_FormalParam_s *v = NULL;

	O7C_NEW(&v, Ast_FormalParam_s);
	NodeInit(&(*v)._._._, Ast_FormalParam_s_tag, Ast_NoId_cnst);
	if (proc->end == NULL) {
		proc->params = v;
	} else {
...
extern int Ast_ParamAdd(struct Ast_RModule *module, struct Ast_RProcType *proc, int buf_len0, o7c_char buf[/*len0*/], int begin, int end) {
	int err = O7C_INT_UNDEF;

	if (SearchName(&proc->params->_._, buf_len0, buf, begin, end) != NULL) {
		err = Ast_ErrDeclarationNameDuplicate_cnst;
	} else if (!(o7c_is(proc, FormalProcType_s_tag)) && (DeclarationLineSearch(&module->_, buf_len0, buf, begin, end) != NULL)) {
		err = Ast_ErrDeclarationNameHide_cnst;
	} else {
		err = Ast_ErrNo_cnst;
	}
	ParamAddPredefined(proc, NULL, false);
	Ast_PutChars(module, &proc->end->_._.name, StringStore_String_tag, buf_len0, buf, begin, end);
	return err;
}
...
extern int Ast_ProcTypeSetReturn(struct Ast_RProcType *proc, struct Ast_RType *type) {
	...
	if (!(o7c_in(type->_._.id, ((1 << Ast_IdArray_cnst) | (1 << Ast_IdRecord_cnst))))) {
		err = Ast_ErrNo_cnst;
	} else {
...
extern void Ast_AddError(struct Ast_RModule *m, int error, int line, int column, int tabs) {
	...
	O7C_NEW(&e, Ast_Error_s);
	NodeInit(&(*e)._, Ast_Error_s_tag, Ast_NoId_cnst);
	e->next = NULL;
	e->code = error;
	e->line = line;
	e->column = column;
...
extern int Ast_MultArrayLenByExpr(int *size, struct Ast_RExpression *e) {
	int i = O7C_INT_UNDEF, err = O7C_INT_UNDEF;
...
extern int Ast_PointerSetType(struct Ast_RPointer *tp, struct Ast_RType *subtype) {
	...
	if (o7c_in(subtype->_._.id, ((1 << Ast_IdRecord_cnst) | (1 << Ast_IdRecordForward_cnst)))) {
		Ast_PointerSetRecord(tp, O7C_GUARD(Ast_Record_s, &subtype));
		err = Ast_ErrNo_cnst;
	} else {
...
static void RecNew(struct Ast_Record_s **r, int id) {
	assert(o7c_in(id, ((1 << Ast_IdRecord_cnst) | (1 << Ast_IdRecordForward_cnst))));
	O7C_NEW(&(*r), Ast_Record_s);
	TInit(&(*r)->_._, id);
	(*r)->pointer = NULL;
	(*r)->vars = NULL;
	(*r)->base = NULL;
...
extern struct Ast_Record_s *Ast_RecordForwardNew(struct Ast_RDeclarations *ds, int name_len0, o7c_char name[/*len0*/], int begin, int end) {
	struct Ast_Record_s *r = NULL;

	RecNew(&r, Ast_IdRecordForward_cnst);
	DeclConnect(&r->_._._, ds, name_len0, name, begin, end);
	ds->recordForwardCount = o7c_add(ds->recordForwardCount, 1);
	return r;
}

static struct Ast_RDeclaration *SearchPredefined(int buf_len0, o7c_char buf[/*len0*/], int begin, int end) {
	struct Ast_RDeclaration *d = NULL;
	int l = O7C_INT_UNDEF;

	l = Scanner_CheckPredefined(buf_len0, buf, begin, end);
	Log_Str(18, (o7c_char *)"SearchPredefined ");
	Log_Int(l);
	Log_Ln();
...
extern struct Ast_RDeclaration *Ast_DeclarationSearch(struct Ast_RDeclarations *ds, int buf_len0, o7c_char buf[/*len0*/], int begin, int end) {
	struct Ast_RDeclaration *d = NULL;
	...
	if (d == NULL) {
		...
		if ((d == NULL) && (o7c_is(ds, Ast_RModule_tag)) && !O7C_GUARD(Ast_RModule, &ds)->fixed) {
...
extern struct Ast_RType *Ast_TypeErrorNew(void) {
	...
	O7C_NEW(&type, Ast_RType);
	NodeInit(&(*type)._._, Ast_RType_tag, Ast_IdError_cnst);
	DeclInit(&type->_, NULL);
	return type;
}

extern struct Ast_RDeclaration *Ast_DeclErrorNew(struct Ast_RDeclarations *ds, int buf_len0, o7c_char buf[/*len0*/], int begin, int end) {
	struct Ast_RDeclaration *d = NULL;

	O7C_NEW(&d, Ast_RDeclaration);
	NodeInit(&(*d)._, Ast_RDeclaration_tag, Ast_IdError_cnst);
	DeclConnect(d, ds, buf_len0, buf, begin, end);
	d->type = Ast_TypeErrorNew();
	return d;
}

extern int Ast_DeclarationGet(struct Ast_RDeclaration **d, struct Ast_RDeclarations *ds, int buf_len0, o7c_char buf[/*len0*/], int begin, int end) {
	int err = O7C_INT_UNDEF;

	(*d) = Ast_DeclarationSearch(ds, buf_len0, buf, begin, end);
	if ((*d) == NULL) {
		if ((ds->_.module != NULL) && o7c_bl(ds->_.module->script)) {
			err = Ast_ImportAdd(ds->_.module, buf_len0, buf, begin, end, begin, end);
			(*d) = ds->end;
		}
		if ((*d) == NULL) {
...
extern int Ast_VarGet(struct Ast_RVar **v, struct Ast_RDeclarations *ds, int buf_len0, o7c_char buf[/*len0*/], int begin, int end) {
	int err = O7C_INT_UNDEF;
	struct Ast_RDeclaration *d = NULL;
...
extern int Ast_ForIteratorGet(struct Ast_RVar **v, struct Ast_RDeclarations *ds, int buf_len0, o7c_char buf[/*len0*/], int begin, int end) {
	int err = O7C_INT_UNDEF;
...
static void ExprInit(struct Ast_RExpression *e, int id, struct Ast_RType *t) {
	NodeInit(&(*e)._, Ast_RExpression_tag, id);
	e->type = t;
	e->value_ = NULL;
}
...
extern struct Ast_ExprReal_s *Ast_ExprRealNew(double real, struct Ast_RModule *m, int buf_len0, o7c_char buf[/*len0*/], int begin, int end) {
	struct Ast_ExprReal_s *e = NULL;

	assert(m != NULL);
	O7C_NEW(&e, Ast_ExprReal_s);
	ExprInit(&e->_._._, Ast_IdReal_cnst, Ast_TypeGet(Ast_IdReal_cnst));
	e->real = real;
	e->_._._.value_ = (&(e)->_._);
	Ast_PutChars(m, &e->str, StringStore_String_tag, buf_len0, buf, begin, end);
	return e;
}
...
extern struct Ast_ExprBoolean_s *Ast_ExprBooleanGet(o7c_bool bool_) {
	struct Ast_ExprBoolean_s *e = NULL;

	e = booleans[o7c_ind(2, (int)bool_)];
	if (e == NULL) {
		O7C_NEW(&e, Ast_ExprBoolean_s);
		ExprInit(&e->_._, Ast_IdBoolean_cnst, Ast_TypeGet(Ast_IdBoolean_cnst));
		e->bool_ = bool_;
		e->_._.value_ = (&(e)->_);
		booleans[o7c_ind(2, (int)bool_)] = e;
	}
	return e;
}

extern struct Ast_ExprString_s *Ast_ExprStringNew(struct Ast_RModule *m, int buf_len0, o7c_char buf[/*len0*/], int begin, int end) {
...
extern struct Ast_RExpression *Ast_ExprErrNew(void) {
	...
	O7C_NEW(&e, Ast_RExpression);
	ExprInit(e, Ast_IdError_cnst, Ast_TypeErrorNew());
	return e;
}
...
extern int Ast_ExprNegateNew(struct Ast_ExprNegate_s **neg, struct Ast_RExpression *expr) {
	...
	if ((expr->type != NULL) && (o7c_cmp(expr->type->_._.id, Ast_IdBoolean_cnst) !=  0)) {
		...
		err = Ast_ErrNo_cnst;
		if (expr->value_ != NULL) {
			(*neg)->_._.value_ = (&(Ast_ExprBooleanGet(!O7C_GUARD(Ast_ExprBoolean_s, &expr->value_)->bool_))->_);
		}
	}
	return err;
}
...
extern int Ast_CheckDesignatorAsValue(struct Ast_Designator_s *d) {
	...
	if ((o7c_is(d->decl, Ast_RVar_tag)) && ((d->decl->up != NULL) && (d->decl->up->_.up != NULL) || (o7c_is(d->decl, Ast_FormalParam_s_tag))) && !O7C_GUARD(Ast_RVar, &d->decl)->inited) {
...
static void SelInit(struct Ast_RSelector *s) {
	NodeInit(&(*s)._, Ast_RSelector_tag, Ast_NoId_cnst);
	s->next = NULL;
}
...
static struct Ast_RVar *RecordVarSearch(struct Ast_Record_s *r, int name_len0, o7c_char name[/*len0*/], int begin, int end) {
	struct Ast_RDeclaration *d = NULL;
	struct Ast_RVar *v = NULL;
...
static struct Ast_RVar *RecordChecklessVarAdd(struct Ast_Record_s *r, int name_len0, o7c_char name[/*len0*/], int begin, int end) {
	struct Ast_RVar *v = NULL;
	...
	Ast_PutChars(v->_.module, &v->_.name, StringStore_String_tag, name_len0, name, begin, end);
	if (r->vars == NULL) {
		r->vars = v;
	} else {
		last = (&(r->vars)->_);
...
extern int Ast_RecordVarAdd(struct Ast_RVar **v, struct Ast_Record_s *r, int name_len0, o7c_char name[/*len0*/], int begin, int end) {
	int err = O7C_INT_UNDEF;
...
extern int Ast_RecordVarGet(struct Ast_RVar **v, struct Ast_Record_s *r, int name_len0, o7c_char name[/*len0*/], int begin, int end) {
	int err = O7C_INT_UNDEF;
...
extern int Ast_SelRecordNew(struct Ast_RSelector **sel, struct Ast_RType **type, int name_len0, o7c_char name[/*len0*/], int begin, int end) {
	struct Ast_SelRecord_s *sr = NULL;
	int err = O7C_INT_UNDEF;
	struct Ast_Record_s *record = NULL;
	struct Ast_RVar *var_ = NULL;

	O7C_NEW(&sr, Ast_SelRecord_s);
	SelInit(&sr->_);
	var_ = NULL;
	err = Ast_ErrNo_cnst;
	if ((*type) != NULL) {
...
static o7c_bool CompatibleTypes_EqualProcTypes(struct Ast_RProcType *t1, struct Ast_RProcType *t2) {
...
extern o7c_bool Ast_CompatibleTypes(int *distance, struct Ast_RType *t1, struct Ast_RType *t2) {
	...
	if (!comp) {
		comp = t1 == t2;
		Log_Str(43, (o7c_char *)"Идентификаторы типов : ");
		Log_Int(t1->_._.id);
		Log_Str(4, (o7c_char *)" : ");
		Log_Int(t2->_._.id);
		Log_Ln();
		if (o7c_bl(comp)) {
		} else if ((o7c_cmp(t1->_._.id, t2->_._.id) ==  0) && (o7c_in(t1->_._.id, ((1 << Ast_IdArray_cnst) | (1 << Ast_IdPointer_cnst) | (1 << Ast_IdRecord_cnst) | (1 << Ast_IdProcType_cnst))))) {
			switch (t1->_._.id) {
			case 7:
				comp = Ast_CompatibleTypes(&(*distance), t1->_.type, t2->_.type);
				break;
			case 6:
				comp = (t1->_.type == t2->_.type) || (t1->_.type == NULL) || (t2->_.type == NULL) || Ast_IsRecordExtension(&(*distance), O7C_GUARD(Ast_Record_s, &t1->_.type), O7C_GUARD(Ast_Record_s, &t2->_.type));
				break;
			case 8:
				/* TODO */
				...
				break;
			case 10:
				comp = CompatibleTypes_EqualProcTypes(O7C_GUARD(Ast_RProcType, &t1), O7C_GUARD(Ast_RProcType, &t2));
				break;
			default:
				abort();
				break;
			}
		} else if (o7c_cmp(t1->_._.id, Ast_IdProcType_cnst) ==  0) {
			comp = (o7c_cmp(t2->_._.id, Ast_IdPointer_cnst) ==  0) && (t2->_.type == NULL);
		} else if (o7c_cmp(t2->_._.id, Ast_IdProcType_cnst) ==  0) {
			comp = (o7c_cmp(t1->_._.id, Ast_IdPointer_cnst) ==  0) && (t1->_.type == NULL);
		}
	}
	return comp;
}
...
extern int Ast_ExprIsExtensionNew(struct Ast_ExprIsExtension_s **e, struct Ast_RExpression **des, struct Ast_RType *type) {
	...
	O7C_NEW(&(*e), Ast_ExprIsExtension_s);
...
static o7c_bool CompatibleAsCharAndString(struct Ast_RType *t1, struct Ast_RExpression **e2) {
	...
	Log_Str(29, (o7c_char *)"1 CompatibleAsCharAndString ");
	...
	if ((*e)->_.type != NULL) {
		if (!(o7c_in((*e)->_.type->_._.id, ((1 << Ast_IdByte_cnst) | (1 << Ast_IdInteger_cnst) | (1 << Ast_IdReal_cnst) | (1 << Ast_IdSet_cnst)))) && (o7c_cmp(add,  - 1) !=  0)) {
			...
			case 5:
				...
				break;
			case 1:
				(*e)->_.value_ = (&(Ast_ExprBooleanGet(O7C_GUARD(Ast_ExprBoolean_s, &term->value_)->bool_))->_);
				break;
			default:
				abort();
				break;
...
extern int Ast_ExprSumAdd(struct Ast_RExpression *fullSum, struct Ast_ExprSum_s **lastAdder, int add, struct Ast_RExpression *term) {
	...
	if ((fullSum != NULL) && (term != NULL) && ExprSumAdd_CheckType(fullSum, term, add, &err) && (fullSum->value_ != NULL) && (term->value_ != NULL)) {
		if (o7c_cmp(add, Scanner_Or_cnst) ==  0) {
			if (o7c_bl(O7C_GUARD(Ast_ExprBoolean_s, &term->value_)->bool_)) {
...
static int MultCalc(struct Ast_RExpression *res, int mult, struct Ast_RExpression *b) {
	...
	if (MultCalc_CheckType(res, b, mult, &err) && (res->value_ != NULL) && (b->value_ != NULL)) {
		...
		case 4:
			...
			break;
		case 1:
			if (o7c_bl(O7C_GUARD(Ast_ExprBoolean_s, &res->value_)->bool_) && !O7C_GUARD(Ast_ExprBoolean_s, &b->value_)->bool_) {
				res->value_ = b->value_;
			}
			break;
		case 5:
...
static int ExprTermGeneral(struct Ast_ExprTerm_s **e, struct Ast_RExpression *result, struct Ast_RFactor *factor, int mult, struct Ast_RExpression *factorOrTerm) {
	struct Ast_RType *t = NULL;
	struct Ast_RFactor *val = NULL;
	int err = O7C_INT_UNDEF;

	assert((o7c_cmp(mult, Scanner_MultFirst_cnst) >=  0) && (o7c_cmp(mult, Scanner_MultLast_cnst) <=  0));
	assert((o7c_cmp(factorOrTerm->_.id, Ast_IdError_cnst) ==  0) || (o7c_is(factorOrTerm, Ast_RFactor_tag)) || (o7c_is(factorOrTerm, Ast_ExprTerm_s_tag)));
	if (factor != NULL) {
		t = factor->_.type;
		val = factor->_.value_;
		if ((t != NULL) && (o7c_cmp(t->_._.id, Ast_IdByte_cnst) ==  0)) {
	...
	(*e)->expr = factorOrTerm;
	(*e)->factor = factor;
	if (o7c_cmp(factorOrTerm->_.id, Ast_IdError_cnst) !=  0) {
...
static int ExprCallCreate(struct Ast_ExprCall_s **e, struct Ast_Designator_s *des, o7c_bool func) {
	int err = O7C_INT_UNDEF;
	struct Ast_RType *t = NULL;
	struct Ast_RProcType *pt = NULL;

	t = NULL;
	err = Ast_ErrNo_cnst;
	if (des != NULL) {
		Log_Str(30, (o7c_char *)"ExprCallCreate des.decl.id = ");
		...
		if (o7c_cmp(des->decl->_.id, Ast_IdError_cnst) ==  0) {
			...
			if (o7c_is(des->_._.type, Ast_RProcType_tag)) {
				t = des->_._.type->_.type;
				if ((t != NULL) != func) {
					err = o7c_add(Ast_ErrCallIgnoredReturn_cnst, (int)func);
				}
			} else {
...
extern o7c_bool Ast_IsChangeable(struct Ast_RModule *cur, struct Ast_RVar *v) {
	Log_StrLn(13, (o7c_char *)"IsChangeable");
...
extern o7c_bool Ast_IsVar(struct Ast_RExpression *e) {
	Log_Str(15, (o7c_char *)"IsVar: e.id = ");
	Log_Int(e->_.id);
	Log_Ln();
	return (o7c_is(e, Ast_Designator_s_tag)) && (o7c_is(O7C_GUARD(Ast_Designator_s, &e)->decl, Ast_RVar_tag));
}

extern int Ast_ProcedureAdd(struct Ast_RDeclarations *ds, struct Ast_RProcedure **p, int buf_len0, o7c_char buf[/*len0*/], int begin, int end) {
	int err = O7C_INT_UNDEF;

	err = CheckNameDuplicate(ds, buf_len0, buf, begin, end);
	O7C_NEW(&(*p), Ast_RProcedure);
	NodeInit(&(*(*p))._._._._, Ast_RProcedure_tag, Ast_IdProcType_cnst);
	DeclarationsConnect(&(*p)->_._, ds, buf_len0, buf, begin, end);
	(*p)->_.header = Ast_ProcTypeNew(false);
	(*p)->_.return_ = NULL;
...
static o7c_bool CallParamNew_TypeVariation(struct Ast_ExprCall_s *call, struct Ast_RType *tp, struct Ast_FormalParam_s *fp) {
	...
	if (o7c_bl(comp)) {
		...
		if (o7c_cmp(id, Scanner_New_cnst) ==  0) {
			...
			comp = (o7c_cmp(id, Scanner_Ord_cnst) ==  0) && (o7c_in(tp->_._.id, ((1 << Ast_IdChar_cnst) | (1 << Ast_IdSet_cnst) | (1 << Ast_IdBoolean_cnst))));
		}
	}
	return comp;
}
...
extern int Ast_CallParamNew(struct Ast_ExprCall_s *call, struct Ast_Parameter_s **lastParam, struct Ast_RExpression *e, struct Ast_FormalParam_s **currentFormalParam) {
	...
	if ((*currentFormalParam) != NULL) {
		if (!Ast_CompatibleTypes(&distance, (*currentFormalParam)->_._.type, e->type) && !CompatibleAsCharAndString((*currentFormalParam)->_._.type, &e) && (o7c_bl((*currentFormalParam)->isVar) || !CompatibleAsIntAndByte((*currentFormalParam)->_._.type, e->type)) && !CallParamNew_TypeVariation(call, e->type, (*currentFormalParam))) {
			err = Ast_ErrCallIncompatibleParamType_cnst;
		} else if (o7c_bl((*currentFormalParam)->isVar)) {
	...
	if ((*lastParam) == NULL) {
		O7C_NEW(&(*lastParam), Ast_Parameter_s);
	} else {
		assert((*lastParam)->next == NULL);
		O7C_NEW(&(*lastParam)->next, Ast_Parameter_s);
		(*lastParam) = (*lastParam)->next;
	}
	NodeInit(&(*(*lastParam))._, Ast_Parameter_s_tag, Ast_NoId_cnst);
	(*lastParam)->expr = e;
	...
	if ((currentFormalParam == NULL) && (o7c_is(call->designator->decl, Ast_PredefinedProcedure_s_tag)) && (call->designator->decl->type->_.type != NULL)) {
		...
		if ((v != NULL) && (o7c_cmp(call->designator->decl->_.id, Scanner_Len_cnst) !=  0)) {
			...
			case 90:
				...
				break;
			case 107:
				call->_._.value_ = (&(Ast_ExprBooleanGet((O7C_GUARD(Ast_RExprInteger, &v)->int_ % 2 == 1)))->_);
				break;
			case 105:
			...
			case 108:
				...
				} else if (o7c_cmp(v->_.type->_._.id, Ast_IdError_cnst) !=  0) {
					Log_Str(40, (o7c_char *)"Неправильный id типа = ");
...
static void StatInit(struct Ast_RStatement *s, struct Ast_RExpression *e) {
	NodeInit(&(*s)._, Ast_RStatement_tag, Ast_NoId_cnst);
	s->expr = e;
	s->next = NULL;
}
...
extern int Ast_CallNew(struct Ast_Call_s **c, struct Ast_Designator_s *des) {
	...
	O7C_NEW(&(*c), Ast_Call_s);
	StatInit(&(*c)->_, &e->_._);
	return err;
}

extern int Ast_CallBeginGet(struct Ast_Call_s **call, struct Ast_RModule *m, o7c_bool acceptParams, int name_len0, o7c_char name[/*len0*/], int begin, int end) {
	struct Ast_RDeclaration *d = NULL;
	...
	} else if (O7C_GUARD(Ast_RProcedure, &d)->_.header->_._._.type != NULL) {
		err = Ast_ErrProcNotCommandHaveReturn_cnst;
	} else if (O7C_GUARD(Ast_RProcedure, &d)->_.header->params != NULL) {
...
extern int Ast_CommandGet(struct Ast_Call_s **call, struct Ast_RModule *m, int name_len0, o7c_char name[/*len0*/], int begin, int end) {
...
extern int Ast_CaseLabelNew(struct Ast_CaseLabel_s **label, int id, int value_) {
	...
	O7C_NEW(&(*label), Ast_CaseLabel_s);
	NodeInit(&(*(*label))._, Ast_CaseLabel_s_tag, id);
	(*label)->qual = NULL;
	(*label)->value_ = value_;
	(*label)->right = NULL;
	(*label)->next = NULL;
...
static o7c_bool IsRangesCross(struct Ast_CaseLabel_s *l1, struct Ast_CaseLabel_s *l2) {
	...
	Log_Str(15, (o7c_char *)"IsRangesCross ");
	Log_Bool(cross);
	Log_Ln();
	return cross;
}
...
static o7c_bool IsListCrossRange(struct Ast_CaseLabel_s *list, struct Ast_CaseLabel_s *range) {
	...
	Log_Str(18, (o7c_char *)"IsListCrossRange ");
	Log_Bool(list != NULL);
	Log_Ln();
	return list != NULL;
}
...
static o7c_bool IsElementsCrossRange(struct Ast_CaseElement_s *elem, struct Ast_CaseLabel_s *range) {
	...
	Log_Str(22, (o7c_char *)"IsElementsCrossRange ");
	Log_Bool(elem != NULL);
	Log_Ln();
	return elem != NULL;
}
...
extern struct Ast_CaseElement_s *Ast_CaseElementNew(struct Ast_CaseLabel_s *labels) {
	...
	O7C_NEW(&elem, Ast_CaseElement_s);
	NodeInit(&(*elem)._, Ast_CaseElement_s_tag, Ast_NoId_cnst);
	elem->next = NULL;
	elem->labels = labels;
	elem->stats = NULL;
	return elem;
...
extern struct Ast_StatementError_s *Ast_StatementErrorNew(void) {
	...
	O7C_NEW(&s, Ast_StatementError_s);
	StatInit(&s->_, NULL);
	return s;
}
...
static void PredefinedDeclarationsInit_TypeNew(int s, int t) {
	...
	if (true || (o7c_cmp(s, Scanner_Byte_cnst) !=  0)) {
		/* TODO */
		O7C_NEW(&td, Ast_RType);
	} else {
		O7C_NEW(&tb, Ast_Byte_s);
		td = (&(tb)->_);
	}
	TInit(td, t);
...
static struct Ast_RProcType *PredefinedDeclarationsInit_ProcNew(int s, int t) {
	...
	O7C_NEW(&td, Ast_PredefinedProcedure_s);
	NodeInit(&(*td)._._._._, Ast_PredefinedProcedure_s_tag, s);
	...
	td->_.header = Ast_ProcTypeNew(false);
...
static void PredefinedDeclarationsInit(void) {
	...
	O7C_NEW(&types[Ast_IdPointer_cnst], Ast_RType);
	NodeInit(&(*types[Ast_IdPointer_cnst])._._, Ast_RType_tag, Ast_IdPointer_cnst);
...
extern void Ast_init(void) {
	...
	if (0 == initialized) {
		...
		o7c_tag_init(Ast_RProcType_tag, Ast_RConstruct_tag);
		o7c_tag_init(FormalProcType_s_tag, Ast_RProcType_tag);
