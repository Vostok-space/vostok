...
static int Read(struct V_Base *in_, o7_tag_t in__tag, int buf_len0, char unsigned buf[/*len0*/], int ofs, int count) {
	return CFiles_Read(O7_GUARD_R(VFileStream_RIn, &(*in_), in__tag).file, buf_len0, buf, ofs, count);
}

static int ReadChars(struct V_Base *in_, o7_tag_t in__tag, int buf_len0, o7_char buf[/*len0*/], int ofs, int count) {
...
extern struct VFileStream_RIn *VFileStream_OpenIn(int name_len0, o7_char name[/*len0*/]) {
	...
	if (in_ != NULL) {
		...
		if (file == NULL) {
			...
			VDataStream_InitIn(&(*O7_REF(in_))._, Read, ReadChars);
...
static int Write(struct V_Base *out, o7_tag_t out_tag, int buf_len0, char unsigned buf[/*len0*/], int ofs, int count) {
	return CFiles_Write(O7_GUARD_R(VFileStream_ROut, &(*out), out_tag).file, buf_len0, buf, ofs, count);
}

static int WriteChars(struct V_Base *out, o7_tag_t out_tag, int buf_len0, o7_char buf[/*len0*/], int ofs, int count) {
...
extern struct VFileStream_ROut *VFileStream_OpenOut(int name_len0, o7_char name[/*len0*/]) {
	...
	if (out != NULL) {
		...
		if (file == NULL) {
			...
			VDataStream_InitOut(&(*O7_REF(out))._, Write, WriteChars);
