...
BEGIN
S("Транслятор Oberon-07 в C и Java. 2018");
S("Использование: ");
S(" 1) o7c help");
S(" 2) o7c to-c     Код ВыхКат {-m ПкМ | -i ПкИ | -infr Инфр}");
S(" 3) o7c to-bin   Код Исполн {-m ПМ|-i ПИ|-infr И|-c ПHC|-cc Компил|-t ВремКат}");
S(" 4) o7c run      Код {-m ПкМ|-i ПкИ|-c ПHC|-infr И|-cc К|-t ВремКат} [-- Пар-ры]");
S(" 5) o7c to-java  Код ВыхКат {-m ПкМ | -i ПкИ | -infr Инфр}");
S(" 6) o7c to-class Код Исполн {-m ПМ|-i ПИ|-infr И|-j ПJ|-javac Компил|-t ВремКат}");
S(" 7) o7c run-java Код {-m ПкМ|-i ПкИ|-c ПHC|-t ВремКат} [-- Пар-ры]");
IF full THEN
S("");
...
S("5) to-java  преобразовывает модули в набор .java файлов");
S("6) to-class превращает модули в .class файлы через неявные .java файлы");
...
S("-c ПJ - Путь к каталогу с .java файлами-воплощениями интерфейсных модулей");
S("  Пример: -c singularity/implementation.java");
