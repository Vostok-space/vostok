...
BEGIN
	...
	| Ast.ErrProcNestedTooDeep:
		O("Слишком глубокая вложенность подпрограмм")
	END
END AstError;
...
PROCEDURE Usage*;
BEGIN
S("Транслятор из Oberon-07 в C. 2017");
S("Использование: ");
S("  1) o7c help");
...
S("  3) o7c to-bin ком-да результат {-m пКм|-i кИм|-c .h,c-файлы} [-t вк] [-cc компил.]");
...
S("  4) o7c run команда {-m путь_к_м.|-i к.с_инт_м.|-c .h,c-файлы} [-t времКат] [-- пар-ры]");
