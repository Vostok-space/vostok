...
S(" 6) ost to-jar   Код Jar {-m TМ|-i ТІ|-infr І|-jv ТJ|-javac Компіл|-t ТмчТк}");
S(" 7) ost run-java Код {-m ТзМ|-i TзІ|-jv ТJ|-t ТимчТек} [-- Пар-ри]");
S(" 8) ost to-js    Код ВихТек {-m ТзМ | -i ТзІ | -infr Інфр}");
...
S("6) to-jar   перетворює модулі в .jar файл крізь неявні .java файли");
S("7) run-java запускає неявний клас з main, створений по Коду");
S("8) to-js    перетворює модулі в набір .js файлів");
...
S("ВихТек - Вихідна Тека для створених файлів.");
S("         може бути замінена на - або пустий аргумент для стандартного виводу");
...
S("");
S("-allow-system  дозволяє небезпечний платформоспецифічний псевдомодуль");
S("");
...
S("-out:O7|-out:AO|-out:CP       - вибір діалекту Oberon для генеруємих модулів");
S("");
...
S("");
S("-native-string - генерувати більш природні строки для цільової мови");
S("");
S("Інтерфейс користувача:");
S("-msg-lang:(eng|rus|ukr) - встановити мову повідомлень транслятора")
END
END Usage;
