...
result/ost to-c "RepeatTran.Go(10)" $RESULT/san  -infr . -m source -m test/benchmark -init noinit -no-array-index-check -no-nil-check -no-arithmetic-overflow-check

export ASAN_OPTIONS=detect_odr_violation=0

compile() {
SI=singularity/implementation
MAIN="gcc -O3 -flto -s -DO7_MEMNG_MODEL=O7_MEMNG_NOFREE -Isingularity/implementation $SI/o7.c $SI/CFiles.c $SI/CLI.c $SI/Platform.c $SI/OsEnv.c $SI/OsExec.c $SI/Unistd_.c $SI/CDir.c $SI/Wlibloaderapi.c"
TRAP="-fsanitize-undefined-trap-on-error -static-libasan"
USAN="-fsanitize=undefined -fno-sanitize=alignment -fno-sanitize=null"
...
run() {
	...
	for ost in $LIST; do
		...
		for i in $@; do
			/usr/bin/time -f "%e sec  %M KiB" $RESULT/$ost to-c "RepeatTran.Go(10)" /tmp/ost-bench-$ost -infr . -m source -m test/benchmark
		done
